...
`;
    ...
    expect(result.primaryDiagnosis?.category).toMatch(/thermal|sensors/);
    expect(result.hypotheses.length).toBeGreaterThan(0);
    expect(result.recommendedTestSequence.length).toBeGreaterThan(0);
    expect(result.detectedSignals.some(s => s.signalKey === 'thermal_sensor_chain_break' || s.signalKey === 'missing_sensors')).toBe(true);
    expect(result.parserSummary.parseReliability).toBeGreaterThan(50);
    expect(result.reasoningTrace[0].stepKey).toBe('parser');
  });
...
`;
    ...
    expect(result.primaryDiagnosis?.supportingEvidence.join(' ')).toContain('codec');
  });
...
`;
    ...
    expect(result.reasoningTrace.some(step => step.kind === 'support')).toBe(true);
  });
...
`;
    ...
    expect(result.secondaryHypotheses.length).toBeGreaterThan(0);
    expect(result.reasoningTrace.some(step => step.kind === 'conflict')).toBe(true);
  });
...
`;
    ...
    expect(result.technicalAlerts.some(a => a.toLowerCase().includes('face id'))).toBe(true);
    expect(result.executiveSummary.toLowerCase().includes('calibração contextual por modelo')).toBe(true);
    expect(result.primaryDiagnosis?.confidenceLabel).toMatch(/moderate|high|very_high/);
  });
...
`;
    ...
    expect(result.technicalAlerts.some(a => a.toLowerCase().includes('modelo legado'))).toBe(true);
  });

  it('downgrades confidence on partial malformed logs without failing', () => {
    const raw = `
panic(cpu 0 caller 0xfffffff): no successful checkins from com.apple.thermalmonitord in 180 seconds
`;
    const result = run(raw);
    expect(result.confidenceScore).toBeGreaterThan(0);
    expect(result.parserSummary.ambiguityLevel).toMatch(/moderate|high/);
